            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="print">
                        <button class="layui-btn layui-btn-xs " type="button" lay-event="print">
...
<script>
    layList.tableList('List',"{:Url('order_list',['real_name'=>$real_name])}",function (){
        return [
            ...
            {field: 'nickname', title: '用户信息',templet:'#userinfo',width:'12%',align:'center'},
            ...
            {field: 'delivery_time', title: '配送时间',width:'8%',sort: true,align:'center'},
            {field: 'print', title: '打印小票',width:'8%',align:'center',toolbar:'#print'},
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            ...
            case 'order_info':
                ...
                break;
            case 'print':
                var url = layList.U({c:'order.store_order',a:'print',p:{id:data.id}});
                $eb.axios.get(url).then(function (res) {
                    if(res.status == 200 && res.data.code == 200) {
                        $eb.$swal('success',res.data.msg);
                    }else
                        return Promise.reject(res.data.msg || '打印失败')
                }).catch(function(err){
                    $eb.$swal('error',err);
                });
                break;
