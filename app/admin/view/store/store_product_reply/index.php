                        ...
                        <div class="media-body" v-for="(item,index) in messageList">
                            ...
                            <div class="message-but">
                                <div class="layui-btn-group">
                                    ...
                                    <button class="layui-btn layui-btn-danger layui-btn-sm" type="button" @click="examine(item,1)" id="op" v-if="item.reply_examine == 0">展示</button>
                                </div>
                            </div>
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            methods:{
                ...
                examine:function(item,flag){
                        var url = layList.U({a:'examine',p:{id:item.id,flag:flag}}),that=this;
                        $eb.$alert('mark',{},function (){
                            $eb.axios.get(url,{flag:flag}).then(function(res){
                                console.log(res);
                                if(res.status == 200 && res.data.code == 200) {
                                    that.$set(that,'messageList',that.messageList);
                                     $("#op").text("已展示").attr('disabled',true).css('background','#9D9D9D');
                                }else
