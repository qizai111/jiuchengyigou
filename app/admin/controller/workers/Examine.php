...
class Examine extends AuthController
{
    ...
    public function worker_list()
    {
        ...
        $count = Db::name('yg_user_data')->count();
        $data = Db::name('yg_user_data')->where($where)->page($input['page'])->order('id', 'desc')
            ->limit(15)->select()->toArray();

        foreach ($data as &$key) {
            $card = explode(',', $key['crad_img']);
            $key['card_z'] = config('oss.YG_URL') . $card[0];
            $key['card_f'] = config('oss.YG_URL') . $card[1];
            $key['certificate'] = config('oss.YG_URL') . $key['certificate'];
