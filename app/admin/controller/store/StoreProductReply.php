...
class StoreProductReply extends AuthController
{
     ...
     * @return \think\response\Json|void
     */
    public function examine($id,$flag){
        if(!$id) return $this->failed('数据不存在');
        if(!$flag) return $this->failed("错误");
        $data['reply_examine'] = $flag;
        $res = ProductReplyModel::edit($data,$id);
        if(!$res)
            return Json::fail("操作失败，稍后再试！");
        else
            return Json::successful("展示成功，其他用户将会看到这条评论。");
