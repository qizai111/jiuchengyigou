...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            Form::number('brass_rebate','青铜会员折扣')->min(0)->max(1)->col(8),
            Form::number('silver_rebate','白银会员折扣')->min(0)->max(1)->col(8),
            Form::number('gold_rebate','黄金会员折扣')->min(0)->max(1)->col(8),
            Form::number('diamond_rebate','钻石会员折扣')->min(0)->max(1)->col(8),
            Form::number('fenxiao_one','一级分销比例')->min(0)->max(1)->col(8),
            Form::number('fenxiao_two','二级分销比例')->min(0)->max(1)->col(8),
            Form::radio('is_show','产品状态',0)->options([['label'=>'上架','value'=>1],['label'=>'下架','value'=>0]])->col(8),
            Form::radio('is_hot','推荐商品',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),
            Form::radio('is_benefit','生态直供',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),//促销单品4
        	Form::radio('is_best','抗疫助农',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8), //精品推荐1
            Form::radio('is_new','农特产品',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),//首发新品3
            Form::radio('is_postage','是否包邮',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),
            Form::radio('is_good','是否今日特价',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8), //今日特价
        ];
    ...
    public function save()
    {
        ...
        if(!$data['brass_rebate'] || !$data['silver_rebate'] || !$data['gold_rebate'] || !$data['diamond_rebate']) return Json::fail('请输入各个等级会员的折扣比例');
        if(!$data['fenxiao_one'] || !$data['fenxiao_two']) return Json::fail('请输入各级分销比例');
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::number('brass_rebate','青铜会员折扣',$product->getData('brass_rebate'))->min(0)->max(1)->col(8),
            Form::number('silver_rebate','白银会员折扣',$product->getData('silver_rebate'))->min(0)->max(1)->col(8),
            Form::number('gold_rebate','黄金会员折扣',$product->getData('gold_rebate'))->min(0)->max(1)->col(8),
            Form::number('diamond_rebate','钻石会员折扣',$product->getData('diamond_rebate'))->min(0)->max(1)->col(8),
            Form::number('fenxiao_one','一级分销比例',$product->getData('fenxiao_one'))->min(0)->max(1)->col(8),
            Form::number('fenxiao_two','二级分销比例',$product->getData('fenxiao_two'))->min(0)->max(1)->col(8),
            Form::radio('is_show','产品状态',$product->getData('is_show'))->options([['label'=>'上架','value'=>1],['label'=>'下架','value'=>0]])->col(8),
            Form::radio('is_hot','推荐商品',$product->getData('is_hot'))->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8), //推荐商品
            Form::radio('is_benefit','生态直供',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),//促销单品4
        	Form::radio('is_best','抗疫助农',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8), //精品推荐1
            Form::radio('is_new','农特产品',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),//首发新品3
            Form::radio('is_postage','是否包邮',$product->getData('is_postage'))->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),
            Form::radio('is_good','是否今日特价',$product->getData('is_good'))->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),//今日特价
        ];
