...
use app\models\user\UserActivitySign;
...
class UserController
{
    ...
    public function address_edit(Request $request)
    {
        ...
        $obj = UserAddress::getLocation($addressInfo['address'][0]);
 /*       if (!isset($addressInfo['address']['province'])) return app('json')->fail('收货地址格式错误!');
        ...
        $addressInfo['province'] = $obj->result->address_components->province;
        $addressInfo['city'] = $obj->result->address_components->city;
        $addressInfo['district'] = $obj->result->address_components->district;
        $addressInfo['is_default'] = (int)$addressInfo['is_default'] == true ? 1 : 0;
        $addressInfo['uid'] = $request->uid();
        $title = $obj->result->title;
       // $addressInfo['detail'] = $title.$addressInfo['detail'];
...
//    }
     ...
     * @throws \think\exception\DbException
     */
    public function sign_egg_config($uid)
    {
        $limit = date("t");
        $signConfig = GroupDataService::getData('sign_egg_num',$limit) ?? [];
        //获取用户签到的号数
        $signList = UserActivitySign::getSignLists($uid);
        for($i=0;$i<count($signConfig);$i++){
            for ($j=0;$j<count($signList);$j++){
                $day = date("d",$signList[$j]['add_time']);
                if (($i+1) == $day){
                    $signConfig[$i]['status'] = 1;
                }
            }
        }
        return $signConfig;
    ...
    public function config(Request $request){
        $signConfig = $this->sign_egg_config($request->uid());
     ...
     * @return mixed
     */
    public function sign_egg_list(Request $request)
    {
        list($page, $limit) = UtilService::getMore([
            ['page', 0],
            ['limit', 0]
        ...
        $signList = UserActivitySign::getSignList($request->uid(), (int)$page, (int)$limit);
     ...
     * @return mixed
     */
    public function sign_egg_integral(Request $request)
    {
        $signed = UserActivitySign::getIsSign($request->uid());
        if ($signed) return app('json')->fail('已签到');
        if (false !== ($egg_num = UserActivitySign::sign($request->uid())))
        {
            $signConfig = $this->sign_egg_config($request->uid());
            $str = '签到获得' . floatval($egg_num["num"]) . $egg_num['unit'].'鸡蛋';
            $sum_sign_day = UserActivitySign::getSignSumDay($request->uid());//统计累计签到天数
            return app('json')->successful("$str", ['number' => $egg_num['num'],'data'=>$signConfig,'sum_sign_day'=>$sum_sign_day]);
        }
        return app('json')->fail(UserActivitySign::getErrorInfo('签到失败'));
    }
    /**
     * 补签功能
     * @param Request $request
    ...
    public function repair_sign(Request $request)
    {
        $repair_day = 1;
        $signConfig = $this->sign_egg_config($request->uid());
        ...
        if($repair_day >= date("d")) return app("json")->fail(UserSign::getErrorInfo('补签的日期超过本日了哦！'));
        if (false !== ($egg_num = UserActivitySign::repairSign($request->uid(),$repair_day))){
            $signConfig = $this->sign_egg_config($request->uid());
            $sum_sign_day = UserActivitySign::getSignSumDay($request->uid());//统计累计签到天数
            $str = '补签获得' . floatval($egg_num["num"]) . $egg_num['unit'].'鸡蛋';
            return app('json')->successful("$str", ['number' => $egg_num['num'],'data'=>$signConfig,'sum_sign_day'=>$sum_sign_day]);
        }
        return app('json')->fail(UserActivitySign::getErrorInfo('补签失败,积分不足！'));
     ...
     * @return mixed
     */
    public function sign_egg_user(Request $request)
    {
        ...
        //是否统计签到
        if ($sign || $all) {
            $user['sum_sign_day'] = UserActivitySign::getSignSumDay($user['uid']);//统计累计签到天数
            $user['is_day_sgin'] = UserActivitySign::getIsSign($user['uid']); //今天是否签到
            $user['is_YesterDay_sgin'] = UserActivitySign::getIsSign($user['uid'], 'yesterday'); //昨天是否签到
            if (!$user['is_day_sgin'] && !$user['is_YesterDay_sgin']) {
                $user['sign_num'] = 0;
            }
        }
        $activity_bill = UserActivityBill::order("id desc")->limit('1')->where("uid",$user['uid'])->find();
        if($activity_bill == null){
            ...
            $activity_bill->toArray();
            $user["number"] = $activity_bill['balance'];
            $user['unit'] = $activity_bill['unit'];
            $user['type'] = $activity_bill['category'] == 'egg' ? "鸡蛋":" ";
        }
        $date1 = strtotime(date("Y-m")."-1");
        $date2 = strtotime(date("Y-m-t 23:59:00"));

        $receive = Db::name("user_receive_bill")->where("uid",$user['uid'])->where("status",1)->whereBetween("receive_time","$date1,$date2")->find();
        if($receive== null){
            $user['receive_status'] = 0;
        }
        else{
           
            $user['receive_status'] = $receive['status'];
        }
        if($request)
     ...
     * @return mixed
     */
    public function sign_egg_month(Request $request)
    {
        list($page, $limit) = UtilService::getMore([
            ['page', 0],
            ['limit', 0]
        ...
        if (!$limit) return app('json')->successful([]);
        $userSignList = UserActivitySign::getSignMonthList($request->uid(), (int)$page, (int)$limit);
     ...
     * @return mixed
     */
    public function sign_receive(Request $request){
        list($user_id, $address_id) = UtilService::getMore([
            ['user_id', 0],
            ['address_id', 0]
        ], $request, true);
        if(!$address_id) return app("json")->fail("请选择正确的地址");
        $userReceive = UserActivitySign::setUserReceive($user_id,$address_id);
        if($userReceive){
            return app('json')->successful("地址保存成功，我们月底会给您配送鸡蛋.");
        }
        else{
            return app("json")->fail("地址保存错误");
        }
    }
