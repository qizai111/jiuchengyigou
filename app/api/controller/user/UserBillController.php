    ...
    public function spread_banner(Request $request)
    {
        ...
        try {
            ...
            if ($type == 1) {
                ...
                if (!$imageInfo) {
                    $res = RoutineCode::getShareCode($user['uid'], 'spread', 'pages/user/user', '');
                    if (!$res) return app('json')->fail('二维码生成失败');
                    $imageInfo = UploadService::instance()->setUploadPath('spread/code')->imageStream($name, $res['res'],$root='', $path='spread/code');
                    if (!is_array($imageInfo)) return app('json')->fail($imageInfo);
                    SystemAttachment::attachmentAdd($imageInfo['name'], $imageInfo['size'], $imageInfo['type'], $imageInfo['dir'], $imageInfo['thumb_path'], 1, $imageInfo['image_type'], $imageInfo['time'], 2,$uid=null);
                ...
                //var_dump(count($routineSpreadBanner));die;
                for($i=0;$i<=count($routineSpreadBanner)-1;$i++){
                    $posterInfo = '海报生成失败:(';
                    $config = array(
                        'image' => array(
                            array(
                        ...
                        'background' => $routineSpreadBanner[$i]['pic']
					);
					//var_dump($routineSpreadBanner[$i]['pic']);die;
						//检测分销海报远程文件是否存在
                    	$spreadInfo = SystemAttachment::get($user['uid'],"uid");
                        if (isset($spreadInfo[$i]['att_dir']) && strstr($spreadInfo[$i]['att_dir'], 'http') !== false && UtilService::CurlFileExist($spreadInfo[$i]['att_dir']) === false) {
                            //若不存在此文件，则删除数据库记录
                            $spreadInfo = null;
                            SystemAttachment::where(['uid' => $user['uid']])->delete();
                        }
                            if(!$spreadInfo || count($spreadInfo) < 3){

	                            $resRoutine = $resRoutine && $posterInfo = UtilService::setSharePoster($config , $path='spread/poster'); //重新生成海报
	                            if (!is_array($posterInfo)) return app('json')->fail($posterInfo);
	                            SystemAttachment::attachmentAdd($posterInfo['name'], $posterInfo['size'], $posterInfo['type'], $posterInfo['dir'], $posterInfo['thumb_path'], 1, $posterInfo['image_type'], $posterInfo['time'], 2,$user['uid']);

	                            if ($resRoutine) {
	                                if ($posterInfo['image_type'] == 1)
	                                    $routineSpreadBanner[$i]['poster'] = $siteUrlHttps . $posterInfo['dir'];
	                                else
	                                    $routineSpreadBanner[$i]['poster'] = UtilService::setHttpType($posterInfo['dir'], 0);
	                                	$routineSpreadBanner[$i]['poster'] = str_replace('\\', '/', $routineSpreadBanner[$i]['poster']);
	                            }
	                        }
	                        elseif(count($spreadInfo) >= 3)
	                            $routineSpreadBanner[$i]['poster'] = $spreadInfo[$i]['att_dir'];
                    }
            } else if ($type == 2) {
                //公众号
            ...
            //var_dump($routineSpreadBanner);die;
