...
use app\api\controller\common\Information;
use app\Request;
...
class AuthController
{
    ...
    public function verify(Request $request)
    {
        ...
        //$data['code'] = $code;
        $res = Information::infor($code, $phone);
       // $res = SMSService::send($phone,SMSService::VERIFICATION_CODE,$data);
        if($res['Code'] != "OK" & $res['Message'] != "OK") return app('json')->fail('短信验证码发送失败'.$res['Message']);
        CacheService::set('code_'.$phone, $code, $time);
        return app('json')->success($res['Message'] ?? '发送失败');
    }
