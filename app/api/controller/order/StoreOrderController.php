...
use app\api\controller\order\StoreRootController as StoreRoot;
...
class StoreOrderController
{
    ...
    public function confirm(Request $request)
    {
        ...
        //获取积分抵扣比例和是否包邮
        $other = [
            'offlinePostage' => sysConfig('offline_postage'),//包邮
            'integralRatio' => sysConfig('integral_ratio') //积分抵用比例
        ];
        $usableCoupon = StoreCouponUser::beUsableCoupon($uid, $priceGroup['totalPrice']);//查询优惠券
        ...
        $data['orderKey'] = StoreOrder::cacheOrderInfo($uid, $cartInfo, $priceGroup, $other);//获取订单号
        ...
        $data['offline_pay_status'] = (int)sysConfig('offline_pay_status') ?? (int)2;//线下支付状态1开启，2关闭
        $data['store_self_mention']= (int)sysConfig('store_self_mention') ?? 0;//门店自提是否开启0关闭，1开启
        $data['system_store'] = ($res = SystemStore::getStoreDispose()) ? $res : [];//门店信息
        //var_dump($data['userInfo']->toArray());die;
...
//        $priceGroup = StoreOrder::getOrderPriceGroup($cartInfo);
    ...
    public function create(Request $request, $key)
    {
        ...
        if (StoreOrder::be(['order_id|unique' => $key, 'uid' => $uid, 'is_del' => 0]))
            return app('json')->status('extend_order', '订单已生成', ['orderId' => $key, 'key' => $key]);
        list($addressId, $couponId, $payType, $useIntegral, $mark, $combinationId, $pinkId, $seckill_id, $formId, $bargainId, $from,$shipping_type,$real_name,$phone,$delivery_time) = UtilService::postMore([
            'addressId', 'couponId', 'payType', ['useIntegral',0], 'mark', ['combinationId', 0], ['pinkId', 0], ['seckill_id', 0], ['formId', ''], ['bargainId', ''], ['from', 'weixin'],
            ['shipping_type',1],['real_name',''],['phone',''],['time','']
        ], $request, true);
        $payType = strtolower($payType);
        if ($bargainId){
            $bargainUserTableId = StoreBargainUser::getBargainUserTableId($bargainId,$uid);//TODO 获取用户参与砍价表编号
        ...
        if ($pinkId){
            //获取团长收货地的坐标
            $k_location = StorePink::getKlocation($pinkId , $fields = "k_latitude,k_longitude");
            //获取用户收货地址的坐标
            $locations = UserAddress::getLocations('id = '.$addressId);
            //计算距离
            //var_dump($k_location);
           //var_dump($locations);die;

            $lang = UserAddress::getDistance($k_location['k_latitude'],$k_location['k_longitude'],$locations['latitude'],$locations['longitude'],2);
            if($lang > 1){
                return app('json')->fail('拼团失败，您与发起拼团团长距离大于1KM。请自行发起拼团');
            }
            if (StorePink::getIsPinkUid($pinkId,  $request->uid()))
                return app('json')->status('ORDER_EXIST', '订单生成失败，你已经在该团内不能再参加了', ['orderId' => StoreOrder::getStoreIdPink($pinkId, $request->uid())]);
            if (StoreOrder::getIsOrderPink($pinkId, $request->uid()))
        ...
        elseif ($from == 'weixinh5')
            $isChannel = 2;
        $order = StoreOrder::cacheKeyCreateOrder($request->uid(), $key, $addressId, $payType, (int)$useIntegral, $couponId, $mark, $combinationId, $pinkId, $seckill_id, $bargainId, false, $isChannel,$shipping_type,$real_name,$phone,$delivery_time);
