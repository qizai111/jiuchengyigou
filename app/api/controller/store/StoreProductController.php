...
class StoreProductController
{
        ...
        try {
            switch ($userType) {
                ...
                case 'routine':
                    ...
                    if (!$imageInfo) {
                       // $data = 'id=' . $id;
                        if ($user['is_promoter'] || sysConfig('store_brokerage_statu') == 2) $data =  'id=' . $id;
                        $res = RoutineCode::getShareCode($user['uid'], 'product_spread', 'pages/user/user', '');
                        //$res = \app\models\routine\RoutineCode::getPageCode('pages/goods_details/index', $data, 280);
                        if (!$res) return app('json')->fail('二维码生成失败');
                        $imageInfo = \crmeb\services\UploadService::instance()->setUploadPath('routine/product')->imageStream($name, $res['res'] , $root='',$path='routine/product');
                        ...
                        $sys = SystemAttachment::attachmentAdd($imageInfo['name'], $imageInfo['size'], $imageInfo['type'], $imageInfo['dir'], $imageInfo['thumb_path'], 1, $imageInfo['image_type'], $imageInfo['time'], 2 ,$uid = null);
                    	file_put_contents('sys.txt',$sys);
                        $rou = RoutineQrcode::setRoutineQrcodeFind($res['id'], ['status' => 1, 'url_time' => time(), 'qrcode_url' => $imageInfo['dir']]);
                        file_put_contents('rou.txt',$rou);
                        $url = $imageInfo['dir'];
    ...
    public function detail(Request $request, $id, $type = 0)
    {
        ...
        $rebate = (float)StoreProduct::getRebate($id,$uid);//获取商品会员折扣比例
        $attrVip=StoreProduct::setLevelPrice($productValue,$uid,false,$rebate);
        $data['storeInfo'] = StoreProduct::setLevelPrice($storeInfo, $uid, true, $rebate);//获取商品其他规格的会员价格
        ...
        if ($uid) {
            ...
            if ($storeBrokerageStatus == 2)
                $data['priceName'] = StoreProduct::getPacketPrice($storeInfo, $attrVip);
            else {
                $user = $request->user();
                if ($user->is_promoter)
                    $data['priceName'] = StoreProduct::getPacketPrice($storeInfo, $attrVip);
    ...
    public function groom_list(Request $request, $type)
    {
        ...
        } else if ($type == 3) {//TODO 抗疫助农
            $info['banner'] = GroupDataService::getData('routine_home_new_banner') ?: [];//TODO 抗疫助农推荐图片
            $info['list'] = StoreProduct::getNewProduct('id,image,store_name,cate_id,price,ot_price,unit_name,sort,IFNULL(sales,0) + IFNULL(ficti,0) as sales', 0, $request->uid());//TODO 抗疫助农
        } else if ($type == 4) {//TODO 促销单品
            $info['banner'] = GroupDataService::getData('routine_home_benefit_banner') ?: [];//TODO 促销单品推荐图片
            $info['list'] = StoreProduct::getBenefitProduct('id,image,store_name,cate_id,price,ot_price,stock,unit_name,sort,sales,ficti');//TODO 促销单品
        }
