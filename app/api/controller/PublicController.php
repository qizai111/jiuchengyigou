...
class PublicController
{
    ...
    public function index(Request $request)
    {
        ...
        $info['firstInfo'] = $routine_index_page[0]['first_info'] ?? '';//sys_config('first_info');//TODO 抗疫助农简介
        $info['salesInfo'] = $routine_index_page[0]['sales_info'] ?? '';//sys_config('sales_info');//TODO 生态特供简介
        $logoUrl = sys_config('routine_index_logo');//TODO 生态特供简介
        ...
        $firstNumber = $routine_index_page[0]['first_number'] ?? 0;//sys_config('first_number');//TODO 抗疫助农个数
        ...
        $info['firstList'] = StoreProduct::getNewProduct('id,image,store_name,cate_id,price,unit_name,IFNULL(sales,0) + IFNULL(ficti,0) as sales', (int)$firstNumber, $request->uid(), false);//TODO 抗疫助农个数
        $info['bastBanner'] = GroupDataService::getData('routine_home_bast_banner') ?? [];//TODO 首页精品推荐图片
        $benefit = StoreProduct::getBenefitProduct('id,image,store_name,cate_id,price,ot_price,stock,sales,ficti,unit_name', 4);//TODO 今日特价直供
        $lovely = GroupDataService::getData('routine_home_new_banner') ?: [];//TODO 抗疫助农顶部图
        $likeInfo = StoreProduct::getHotProduct('id,image,store_name,cate_id,price,unit_name', 4);//TODO 热门榜单 猜你喜欢
        $goods = StoreProduct::getGoodProduct('id,image,store_name,cate_id,price,unit_name', 4);//TODO 今日特价 优品推荐
        ...
        return app('json')->successful(compact('banner', 'menus', 'roll', 'info', 'activity', 'goods','lovely', 'benefit', 'likeInfo', 'logoUrl', 'couponList', 'site_name', 'subscribe','newGoodsBananr'));
    }
    ...
    public function upload_image(Request $request)
    {
        ...
        $res = UploadService::instance()->setUploadPath('store/comment')->image($data['filename'],$path="store/comment");
	...
	public function checkSignature()
{
    ...
    $token = "023c92ce73cbd7cc87a4a26bd64fc694";
    ...
    if ($tmpStr == $signature ) {
    	...
    	file_put_contents("checkSignature.txt",$_GET);
        return false;
    }
}
