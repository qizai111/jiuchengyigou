...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
       ...
       // $rebate = self::getRebate($list['id'],$uid);
        //return self::setLevelPrice($list, $uid,false, $rebate);
        return $list;
    }
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public static function getGoodProduct($field = '*', $limit = 0, $uid = 0)
    {
        $model = self::where('is_good', 1)->where('is_del', 0)->where('mer_id', 0)
            ->where('stock', '>', 0)->where('is_show', 1)->field($field)
            ->order('sort DESC, id DESC');
        if ($limit) $model->limit($limit);
        $list = $model->select();
     ...
     * @return array
     * */
    public static function setLevelPrice($list, $uid, $isSingle = false, $rebate)
    {
        ...
        //$grade = UserLevel::getUserLevel($uid);
        if ($rebate) {
            $discount = bcsub(1, $rebate, 2);
        } else {
            $discount = $rebate;
    ...
    /**
     * 获取商品会员折扣比例
     * @param object | array $pid 商品id
     * @param int $uid 用户uid
     * @return array
     * */
    public static function getRebate($pid,$uid){
        $grade = UserLevel::getUserGrade($uid);
        if($grade){
            //如果是会员返回折扣比例，不是则返回0
            $rebate = DB::name("store_product_level")->where('grade',$grade)->value('level');
            $rebate = self::where('id',$pid)->field("$rebate")->value("$rebate");
        }else{
            $rebate = 0;
        }
        return $rebate;
     ...
     * @return array
     * */
    public static function getVipPrice($list, $uid)
    {
        if (is_object($list)) $list = count($list) ? $list->toArray() : [];
        $price=self::where("id",$list)->field("price")->find()->toArray();
        $price=$price['price'];
        if (!sys_config('vip_open')) { //若未开启会员折扣，则返回原价
            return $price;
        }
        $grade = UserLevel::getUserLevel($uid);//获取到用户vip等级

        if ($grade) {
            $rebate=DB::name('store_product_level')->where('grade',$grade)->find();//获取对应的等级折扣
            $level=$rebate['level'];
            $vip_rebate = self::where('id',$list)->field("$level")->find()->toArray();//获取折扣比例
            $vip_rebate=$vip_rebate[$level];
        } else {
            $vip_rebate = 0;//若不是会员折扣比为 0
        }
        //如果不是数组直接执行减去会员优惠金额
        ...
        if (!is_array($list)){
            ...
            if ($grade)
                //如果$isSingle==true 返回优惠后的总金额，否则返回优惠的金额
                return bcsub($price, bcmul($vip_rebate, $price, 2), 2);
            else
                return $price;
        }else{
            //当$list为数组时$isSingle==true为一维数组 ，否则为二维
            //$list['vip_price'] = isset($list['price']) ? bcsub($list['price'], bcmul($vip_rebate, $list['price'], 2), 2) : 0;
            foreach ($price as &$item) {
                //优惠后的价钱
                $item['vip_price'] = isset($item['price']) ? bcsub($item['price'], bcmul($vip_rebate, $item['price'], 2), 2) : 0;
            }
            return $price;
        }
    }
     ...
     * @return int|string
     */
    public static function getPacketPrice($storeInfo, $attrVip)
    {
       // $store_brokerage_ratio = sys_config('store_brokerage_ratio');
        $store_brokerage_ratio = $storeInfo['fenxiao_one'];
       // $store_brokerage_ratio = bcdiv($store_brokerage_ratio, 100, 2);
        if (count($attrVip)) {
            $Maxkey = self::getArrayMax($attrVip, 'vip_price');
            $Minkey = self::getArrayMin($attrVip, 'vip_price');
            if (isset($attrVip[$Maxkey])) {
                $value = $attrVip[$Maxkey];
                if ($value['cost'] > $value['price'])
                    $maxPrice = 0;
                else
                    $maxPrice = bcmul($store_brokerage_ratio, $value['vip_price'],2);
                unset($value);
            } else $maxPrice = 0;
            if (isset($attrVip[$Minkey])) {
                $value = $attrVip[$Minkey];
                if ($value['cost'] > $value['price'])
                    $minPrice = 0;
                else
                    $minPrice = bcmul($store_brokerage_ratio, $value['vip_price'],2);
                unset($value);
            } else $minPrice = 0;
