...
class StoreCart extends BaseModel
{
    ...
    {
        $productInfoField = 'id,image,price,ot_price,vip_price,postage,give_integral,sales,stock,store_name,unit_name,is_show,is_del,is_postage,cost';//普通商品
        $seckillInfoField = 'id,image,price,ot_price,postage,give_integral,sales,stock,title as store_name,unit_name,is_show,is_del,is_postage,cost';//秒杀商品
        $bargainInfoField = 'id,image,min_price as price,price as ot_price,postage,give_integral,sales,stock,title as store_name,unit_name,status as is_show,is_del,is_postage,cost';//砍价商品
        $combinationInfoField = 'id,image,price,postage,sales,stock,title as store_name,is_show,is_del,is_postage,cost';//拼团商品
        $model = new self();
        $valid = $invalid = [];
        $model = $model->where('uid',$uid)->where('type','product')->where('is_pay',0)
            ->where('is_del',0);// 获取购物车里的商品列表
        if(!$status) $model = $model->where('is_new',0);
        if($cartIds) $model = $model->where('id','IN',$cartIds);
        $model = $model->order('add_time DESC');
        $list = $model->select()->toArray();
       // var_dump($list);die; $list为购物车详情

        if(!count($list)) return compact('valid','invalid');
        //获取购物车里的秒杀、拼团、砍价、普通商品
        foreach ($list as $k=>$cart){
            ...
            //正常商品
                ...
                $rebate = (float)StoreProduct::getRebate($cart['product_id'],$uid);//获取商品的折扣比例
                if($cart['product_attr_unique']){
                    ...
                    if(!$attrInfo || !$attrInfo['stock'])
                        ...
                        if($cart['combination_id'] || $cart['seckill_id'] || $cart['bargain_id']) {
                            ...
                            $cart['truePrice'] = (float)StoreProduct::setLevelPrice($attrInfo['price'],$uid,true, $rebate);//优惠后的价格
                            $cart['vip_truePrice'] = (float)StoreProduct::setLevelPrice($attrInfo['price'], $uid, false,$rebate);//优惠金额
                        }
                    ...
                    if($cart['combination_id'] || $cart['seckill_id'] || $cart['bargain_id']) {
                        ...
                        $cart['truePrice'] = (float)StoreProduct::setLevelPrice($cart['productInfo']['price'],$uid,true, $rebate);//优惠后的价格
                        $cart['vip_truePrice'] = (float)StoreProduct::setLevelPrice($cart['productInfo']['price'], $uid, false,$rebate);//优惠金额
                    }
...
//                $invalid[] = $cart;
    ...
    *  获取购物车内最新一张产品图
     ...
     *  获取某个字段
     */
    public static function getCartField($id,$field=""){
        return self::where('id',$id)->field("$field")->value("$field");
    }
