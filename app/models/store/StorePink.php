...
class StorePink extends BaseModel
{
    ...
    public static function getPinkAll($cid, $isAll = false)
    {
        $model = new self();
        $model = $model->alias('p');
        $model = $model->field('p.id,p.uid,p.people,p.price,p.stop_time,u.nickname,u.avatar,p.k_address');
     ...
     * 获取团长的拼团地址
     */
    public static function getKaddress($pinkId)
    {
        $model = new self();
        $model = $model->where('id',$pinkId )->where('is_refund', 0);
        $address = $model->field("k_address")->value("k_address");
        return str_replace(" ","",$address);
    }
    /**
     * 获取团长的拼团地址坐标
     */
    public static function getKlocation($pinkId,$fields = "*")
    {
        $model = new self();
        $model = $model->where('id',$pinkId )->where('is_refund', 0);
        $location = $model->field("$fields")->find();
    ...
    public static function createPink($order)
    {
        ...
        $locations = UserAddress::getLocations('id = '.$order['address_id']);
        if ($product) {
            if ($order['pink_id']) {//拼团存在
                ...
                $pink['k_id'] = 0;//拼团id
                if($order['pink_id'] == 0){
                    $pink['k_address'] = $order['user_address']; //团长位置
                    $pink['k_latitude'] = $locations['latitude']; //维度
                    $pink['k_longitude'] = $locations['longitude']; //经度
                }
                foreach ($order['cartInfo'] as $v) {
                    $pink['cid'] = $v['combination_id'];//拼团产品id
                    $pink['pid'] = $v['product_id'];//产品id
