...
class StoreOrder extends BaseModel
{
    ...
    public static function getOrderPriceGroup($cartInfo)
    {
        ...
        $totalPrice = self::getOrderSumPrice($cartInfo, 'truePrice');//获取订单优惠后的总金额
        $costPrice = self::getOrderSumPrice($cartInfo, 'costPrice');//获取订单成本价
        $vipPrice = self::getOrderSumPrice($cartInfo, 'vip_truePrice');//获取订单折扣总金额
        //如果满额包邮等于0
        if (!$storeFreePostage) {
            $storePostage = 0;
        } else {
            foreach ($cartInfo as $cart) {
                if (!$cart['productInfo']['is_postage'])//如果产品不包邮
        ...
        $totalPrice = bcsub($totalPrice,$storePostage,2);//获取订单优惠后的总金额加上邮费
    ...
    public static function cacheKeyCreateOrder($uid, $key, $addressId, $payType, $useIntegral = false, $couponId = 0, $mark = '', $combinationId = 0, $pinkId = 0, $seckill_id = 0, $bargain_id = 0, $test = false, $isChannel = 0, $shipping_type = 1, $real_name = '', $phone = '' , $delivery_time = '')
    {
        self::beginTrans();
        try {
...
//        }
    ...
    {
        //3天前时间戳
        $systemDeliveryTime = sys_config('system_delivery_time') ? sys_config('system_delivery_time'):0;
        //0为取消自动收货功能
        if ($systemDeliveryTime == 0) return true;
        //$sevenDay = strtotime(date('Y-m-d H:i:s', strtotime('-' . $systemDeliveryTime . ' day')));
        $threeDay = strtotime('-' . $systemDeliveryTime . ' day');
        $model = new self;
        $model = $model->alias('o');
        $model = $model->join('StoreOrderStatus s', 's.oid=o.id');
        $model = $model->where('o.paid', 1);
     //   $model = $model->where('s.change_type', 'delivery_fictitious');
        $model = $model->where('s.change_time', '<', $threeDay);
