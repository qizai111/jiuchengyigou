...
//                            if(isset($wechatUser['isPromoter'])) $data['is_promoter'] = $wechatUser['isPromoter'] ? 1 : 0;
            ...
            }elseif(!isset($wechatUser['code']) || !$wechatUser['code']){
                $data['spread_uid'] = "5";
                $data['spread_time'] = time();
            }
  ...
  //                          if(isset($wechatUser['isPromoter'])) $data['is_promoter'] = $wechatUser['isPromoter'] ? 1 : 0;
    ...
    public static function setRoutineUser($routineUser, $spread_uid = 0)
    {
        ...
        $spread_uid = $spread_uid? $spread_uid:5;
...
//            'is_promoter'=>$spread_uid || $storeBrokerageStatu != 1 ? 1: 0,
    ...
    public static function backOrderBrokerage($orderInfo,bool $open = true)
    {
        ...
        //TODO 获取商品一级返佣金额 默认0
        $brokeragePrice = 0;
        $order_cart_info = Db::name("store_order_cart_info")->where("oid",$orderInfo['id'])->select()->toArray();
         foreach ($order_cart_info as $k=>$v){
            $info = \Qiniu\json_decode($v['cart_info']);
            //总金额
            $payPrice = bcmul($info->cart_num,$info->truePrice,2);
            //TODO 获取商品一级分销比例
            $storeBrokerageRatio = StoreProduct::getProductField($v['product_id'],"fenxiao_one");//例如0.05
            //TODO 一级返佣比例 小于等于零时直接返回 不返佣
            if ($storeBrokerageRatio <= 0) return true;
            //TODO 返佣金额 = 实际支付金额 * 一级返佣比例
            $brokeragePrice += bcmul($payPrice, $storeBrokerageRatio, 2);
        }
        //TODO 获取订单实际支付金额减去邮费金额
        //$payPrice = bcsub($orderInfo['pay_price'],$orderInfo['pay_postage'],2);
        //TODO 成本价
       ...
       // if ($cost >= $orderInfo['pay_price']) return true;
        ...
        //TODO 添加推广记录
        $res1 = UserBill::income('获得一级推广佣金', $userInfo['spread_uid'], 'now_money', 'brokerage', $brokeragePrice, $orderInfo['id'], $balance, $mark);
        //TODO 添加用户余额
...
//        if($res) return self::backOrderBrokerageTwo($orderInfo);
    ...
    public static function backOrderBrokerageTwo($orderInfo,bool $open = true)
    {
        ...
        foreach ($order_cart_info as $k=>$v){
            ...
            $payPrice = bcmul($info->cart_num,$info->truePrice,2);
            //TODO 获取商品一级分销比例
            $storeBrokerageRatio = StoreProduct::getProductField($v['product_id'],"fenxiao_two");//例如0.05
            //TODO 一级返佣比例 小于等于零时直接返回 不返佣
            if ($storeBrokerageRatio <= 0) return true;
            //TODO 返佣金额 = 实际支付金额 * 一级返佣比例
            $brokeragePriceTwo += bcmul($payPrice, $storeBrokerageRatio, 2);
        }
        //TODO 获取成本价
        //$cost = isset($orderInfo['cost']) ? $orderInfo['cost'] : 0;
        //TODO 获取订单实际支付金额减去支付的邮费
        //$payPrice =  bcsub($orderInfo['pay_price'], $orderInfo['pay_postage'], 2);
        //TODO 返佣金额小于等于0 直接返回不返佣金
        if ($brokeragePriceTwo <= 0) return true;
        //TODO 获取上上级推广员信息
        $spreadUserInfoTwo = User::getUserInfo($userInfoTwo['spread_uid']);
        //TODO 获取上上级推广员返佣之后余额
        $balance = bcadd($spreadUserInfoTwo['brokerage_price'], $brokeragePriceTwo, 2);
        $mark = '二级推广人' . $userInfo['nickname'] . '成功消费' . floatval($orderInfo['pay_price']) . '元,奖励推广佣金' . floatval($brokeragePriceTwo);
        $open && self::beginTrans();
        //TODO 添加返佣记录
        $res1 = UserBill::income('获得二级推广佣金', $userInfoTwo['spread_uid'], 'now_money', 'brokerage', $brokeragePriceTwo, $orderInfo['id'], $balance, $mark);
        //TODO 添加用户余额
        $res2 = self::bcInc($userInfoTwo['spread_uid'], 'brokerage_price', $brokeragePriceTwo, 'uid');
        $res = $res1 && $res2;
        $open && self::checkTrans($res);
        return $res;
    }
    ...
    public static function setSpreadUid($uid, $spreadUid)
    {
        ...
        $spreadUid = $spreadUid ? $spreadUid:5;
