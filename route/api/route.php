...
//    Route::post('like/del', 'user.UserController/like_del')->name('likeDel');//取消点赞
    ...
    //签到领鸡蛋
    Route::get('sign/egg_config', 'user.UserController/config')->name('signConfig');//签到配置
    Route::get('sign/egg_list', 'user.UserController/sign_egg_list')->name('signList');//签到列表
    Route::get('sign/egg_month', 'user.UserController/sign_egg_month')->name('signIntegral');//签到列表（年月）
    Route::post('sign/egg_use', 'user.UserController/sign_egg_user')->name('signUser');//签到用户信息
    Route::post('sign/egg_integral', 'user.UserController/sign_egg_integral')->name('signIntegral');//签到
    Route::post('sign/repair_sign', 'user.UserController/repair_sign')->name('signIntegral');//补签
    Route::post('sign/receive', 'user.UserController/sign_receive')->name('signIntegral');//签到领取记录
    //优惠券类
    ...
    Route::get('user/level/level', 'user.UserLevelController/level')->name('userLevelTask');//获取用户等级
...
Route::group(function () {
    ...
    //用工类
//    Route::get('5d4a418d882b6','employer.Category/index')->name('worker');
    //用工
    Route::rule('workerList', 'employer.category/index', 'GET');
    Route::rule('ChoiceWorker', 'employer.index/ChoiceWorker', 'POST');
    Route::rule('editOrder', 'employer.index/editOrder', 'POST');
    Route::rule('order', 'employer.index/order', 'GET');
    Route::rule('comment', 'employer.index/comment', 'POST');
    Route::rule('details', 'employer.index/details', 'GET');
    //基地展示类
    Route::get('base/show','base.BaseController/baseShow')->name('baseShow');
    Route::get('base/artice','base.BaseController/artice')->name('artice');
    //公共类
    Route::get('checkSignature','PublicController/checkSignature')->name('checkSignature'); //小程序模板消息接口
    //直播类
    Route::get('live/roominfo','live.live/roomInfo')->name('roominfo');
