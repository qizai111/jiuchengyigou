...
class OSS
{
     ...
     * @return string
     */
    public static function uploadImage($filename = 'image',$directory)
    {
        $pid=input("get.pid",0);
        $request = app('request');
        ...
        $object="$directory" . "/" . "$pid" . "/" . date("Ymd") . "/" . $key . $ext;
        try {
            self::autoInfo();
            return self::$auth->uploadFile(self::$storageName, $object, $filePath);
     ...
     * @return string
     */
    public static function uploadImageStream($key, $content,$directory)
    {
    	$object="$directory"."/".date("Ymd")."/".$key;
        try {
            self::autoInfo();
            return self::$auth->putObject(self::$storageName, $object, $content);
        } catch (OssException $e) {
            return $e->getMessage();
        }
    }
